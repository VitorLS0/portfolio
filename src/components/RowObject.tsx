...
function Model({ url, style, scale, onReady }: ModelProps) {
  ...
  useFrame(({ gl, size }, delta) => {
    ...
    if (reducedMotion.matches) {
      ...
      // A finger dragged across the list turns every model together, around
      // the rest pose, instead of each reading the cursor's position.
      y = REST.y + swipe.y * MAX_TURN.y;
    } else {
